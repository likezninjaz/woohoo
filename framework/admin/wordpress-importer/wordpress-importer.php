	...
	class WP_Import extends WP_Importer
	{
		...
		function process_categories() {
			...
			foreach ( $this->categories as $cat ) {
				...
				/*
				 * Added by Aldmohy
				 * Add categories to list in options
				 */
				$demos_categories_id 	= get_option('demos_categories_id');
				$demos_categories_id []	= $id;
				update_option('demos_categories_id', $demos_categories_id);
			}
		...
		function process_posts() {
			...
			foreach ( $this->posts as $post ) {
				...
				/*
				 * Added by Aldmohy
				 * Add meta key for every demo post
				 */
				$demos_posts_id 	= get_option('demos_posts_id');
				$demos_posts_id []	= $post_id;
				update_option('demos_posts_id', $demos_posts_id);
			}
		...
		function process_menu_item( $item ) {
			 ...
			 * https://gist.github.com/mo3aser/c02c2125999c4bc3099f
			 */
			...
			if ( $id && ! is_wp_error( $id ) ) {
				...
				$tie_menu_option = array( '_menu_itemIcon', '_menu_itemType', '_menu_itemDisplay', '_menu_item_color' );
				foreach ( $item['postmeta'] as $meta ){
			...
			/*
			 * Added by Aldmohy
			 * Add menus to list in options
			 */
			$demos_menus_id 	= get_option('demos_menus_id');
			$demos_menus_id []	= $id;
			update_option('demos_menus_id', $demos_menus_id);
		}
...
if( ! function_exists( 'bdaia_wordpress_importer_init' ) )
{
	function bdaia_wordpress_importer_init()
	{
	...
	add_action( 'admin_init', 'bdaia_wordpress_importer_init' );
}