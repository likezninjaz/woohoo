	...
	static function _global_head()
	{
        ...
        <script type="text/javascript">
            ...
            jQuery(function($)
            {
                $(document).click(function(e)
                {
                    ...
                    if (elem.attr('class') && elem.filter('[class*=dodelete]').length)
                    {
                        ...
                        if (confirm('This action can not be undone, are you sure?'))
                        {
                            ...
                            v = elem.parents('.wpa_group').find('input[id^="bdaia_taqyeem_criteria_score"]').val();
                            score = $('.bdaia_taqyeem_all_score').val() - v;
                            $('.bdaia_taqyeem_all_score').val(score);
                ...
                var all_score   = $( '.bdaia_taqyeem_all_score' ).data( 'all_scores'    );
                var score       = $( '.bdaia_taqyeem_all_score' ).data( 'score'         );
                var cscore = 0;

                $('.bdaia_taqyeem_criteria_slider_wrap').each(function()
                {
                    var t       = $(this);
                    var id      = $(this).data('id');
                    var slider  = t.find('#bdaia_taqyeem_criteria_slider_'+id);
                    var input   = t.find('#bdaia_taqyeem_criteria_score_'+id);
                    var min     = 0;
                    var max     = 100;
                    var step    = 1;
                    var value   = input.val();
                    ...
                    slider.slider({
                        ...
                        change: function( event, ui )
                        {
                            ...
                            $('.bdaia_taqyeem_all_score').val(cscore);
                            $('.bdaia_taqyeem_final_score').val(fscore);
                        }
                    });
                ...
                $('[class*=docopy-]').click(function(e)
                {
                    ...
                    ns = $( '.bdaia_taqyeem_all_score' ).val();

                    the_clone.find('.bdaia_taqyeem_criteria_slider_wrap').empty();

                    var bdaia_tcs_id     = the_clone.find('.bdaia_taqyeem_criteria_slider_wrap').data('id');
                    var bdaia_tcs_name   = the_clone.find('.bdaia_taqyeem_criteria_slider_wrap').data('name');

                    the_clone.find('.bdaia_taqyeem_criteria_slider_wrap').append('<div id="bdaia_taqyeem_criteria_slider_'+bdaia_tcs_id+'" class="bdaia_taqyeem_criteria_slider"></div><input type="text" id="bdaia_taqyeem_criteria_score_'+bdaia_tcs_id+'" name="'+bdaia_tcs_name+'" value="0" />' );

                    var jqslider = the_clone.find('.bdaia_taqyeem_criteria_slider_wrap');
                    ...
                    var the_props = ['name', 'id', 'for','class', 'data-id', 'data-name'];
