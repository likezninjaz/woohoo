...
function woohoo_breadcrumbs()
{
	...
	if ( !is_home() && !is_front_page() || is_paged() ) {
		...
		echo '<div xmlns:v="http://rdf.data-vocabulary.org/#"  class="bdaia-crumb-container">';
		
		$homeLink = esc_url(home_url());
		echo '<span typeof="v:Breadcrumb"><a rel="v:url" property="v:title" class="crumbs-home" href="' . $homeLink . '">' . woohoo_lang_ee( 'Home' ) . '</a></span> ' . $delimiter . ' ';
		...
		elseif ( is_search() ) {
			...
			echo $before ;
			printf( woohoo_lang_ee( 'Search Results for:' ) . ' %s',  get_search_query() );
			echo  $after;
		...
		elseif ( is_tag() ) {
			...
			echo $before ;
			printf( woohoo_lang_ee( 'Tag Archives:' ) . ' %s', single_tag_title( '', false ) );
			echo  $after;
			...
			echo $before;
			woohoo_lang_e( 'Not Found' );
			echo  $after;
		}

		if ( get_query_var('paged') ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
			echo woohoo_lang_ee( 'page' ) . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
